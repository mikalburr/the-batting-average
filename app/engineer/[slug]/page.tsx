...
export default async function EngineerPage({ params }: Props) {
  ...
  if (!credit || credit.role !== "ENGINEER") notFound();
    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        <p className="text-xs text-text-muted mb-6">
          Engineers / <span className="text-text-primary">{credit.name}</span>
        </p>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-3">
              ...
              <span className="text-xs text-text-muted">
                Engineer / Mixer · {credit.songs.length} songs
              </span>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="py-2 px-3 text-right text-xs text-text-muted font-body">Eng Score</th>
                    <th className="py-2 px-3 text-right text-xs text-text-muted font-body">Overall</th>
                  </tr>
                </thead>
                <tbody>
                  {credit.songs.map((song) => {
                    const engAvg = song.engineering_score / 100;
                    const engTier = getTier(engAvg);
                    return (
                      <tr key={song.slug} className="border-b border-border hover:bg-raised transition-colors">
                        ...
                        <td className="py-3 px-3 text-right">
                          <span className="font-mono text-sm" style={{ color: "#00E5B0" }}>
                            {fmtAvg(engAvg)}
                          </span>
                        </td>
                        <td className="py-3 px-3 text-right">
